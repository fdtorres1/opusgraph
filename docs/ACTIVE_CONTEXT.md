...
- Public index confidence implementation:
  ...
  - production public seed:
    ...
    - after deployed UI/API smoke tests passed, applied a second conservative ten-work batch with non-duplicate public-list titles and explicit orchestral instrumentation:
      ...
      - `2 Blake Songs`
      - `20 Minuets`
    - all fifteen public-seed works use `promotion_gate_version = 'public-index-gate-v1'`
    - each public-seed work has exactly one `work_promotion_decision`
    - post-second-batch public RPC surface is `15` works through `public_min_works(null, null)` and `25` composers through `public_min_composers(null)`
    - `public_work_detail('824fcc01-fe10-4e53-9912-5cd11edaae26')` returns `1812 Overture`, `indexed`, composer `Tchaikovsky`
    - remaining draft promotion dry-run is now `63` checked, `17` passing, `46` blocked
  - deployed public smoke tests:
    - `/works` renders `15 works found`, including the fifteen public-seed works, with no browser console warnings/errors
    - `/search` renders and searching `1812` returns the `1812 Overture` work result linked to `/works/824fcc01-fe10-4e53-9912-5cd11edaae26`
    - `/api/public/search?q=1812&type=all` returns the indexed `1812 Overture` work
...
- Offset `3500` is now operationally closed:
  - initial dry-run row `41c3d6c1-a613-4fa8-9fb3-d12d3559627e` settled at:
    ...
    - `49` flagged
    - `49` failed
    - all `49` failures were `missing_resolved_composer_id`
  - targeted composer seeding from `scripts/seed-imslp-work-composers.ts --offset 3500 --batch-size 100` then created `46` IMSLP-linked composers:
    - `failedWorkRows = 49`
    - `uniqueMissingComposers = 46`
    - `seedResults = { created: 46, updated: 0, flagged: 0, failed: 0 }`
  - canonical replay dry-run row `99738cfe-2535-4aed-a18a-6f22e7fb7d16` settled green:
    - `100` processed
    - `3` created
    - `97` flagged
    - `0` failed
  ...
  - live row `437d78ca-9802-41d2-8a2f-112f53218058` initially looked stale in the same zero-counter window seen on prior slices, and the first exact coverage audit raced ahead of the delayed live writes:
    - first post-live audit returned:
      ...
      - uncovered outcomes:
        - `28` `quarantined`
        - `1` `created`
  - the live row then eventually settled without any targeted repair:
    - `100` processed
    - `2` created
    - `98` flagged
    - `0` failed
    - `90` rows quarantined
    - cursor advanced to `3600`
  - authoritative post-live verification then passed:
    ...
    - duplicate-flag hygiene remained:
      ...
      - `0` duplicate-source collisions
  - operator note:
    - this slice reconfirmed that early coverage audits can still race delayed live writes even after the broad live row has been created
    - the safer operating pattern remains “wait, verify against exact source-level coverage, and only repair if the uncovered set stays non-zero”
  - next step:
    - ship this `3500` handoff branch, then continue with the offset-`3600` recovery flow from a fresh worktree
...
- Offset `3400` is now operationally closed:
  - initial dry-run row `fb521217-9ec3-469b-8f7b-c020cf9f0559` settled at:
    ...
    - `44` flagged
    - `54` failed
    - all `54` failures were `missing_resolved_composer_id`
  - targeted composer seeding from `scripts/seed-imslp-work-composers.ts --offset 3400 --batch-size 100` then created `51` IMSLP-linked composers:
    - `failedWorkRows = 54`
    - `uniqueMissingComposers = 51`
    - `seedResults = { created: 51, updated: 0, flagged: 0, failed: 0 }`
  - direct post-seed resolution probing then came back green:
    - `unresolvedCount = 0`
  ...
  - the broad live wrapper rows both remained stale in `running` with zero counters:
    - `e709e7db-ebb6-4370-b0aa-95d12ae56782`
    - `9c834ca2-b502-437e-90a4-3230c1fd6a81`
  - exact source-level coverage initially exposed the missing live impact:
    - `69` covered candidates
    - `31` uncovered candidates
    - uncovered dry-run outcomes:
      - `28` `quarantined`
      - `3` `flagged_duplicate`
  - targeted live repair then closed the uncovered set despite the stale wrapper rows
  - authoritative post-repair verification passed:
    ...
    - duplicate-flag hygiene remained:
      ...
      - `0` duplicate-source collisions
  - operator note:
    - this slice reconfirmed that exact source-level coverage is the real gate, not wrapper/job-row cosmetics
    - with `0017` now applied, duplicate hygiene stayed clean even while live wrapper rows went stale
  - next step:
    - ship this `3400` handoff branch, then continue with the offset-`3500` recovery flow from a fresh worktree
...
- Offset `3200` is now operationally recovered:
  - initial dry-run rows `a1396585-b5ad-437c-b695-968646521b2c` and `49b8e73e-afcf-4a91-a098-188c1dccc691` both eventually settled at:
    ...
    - `54` flagged
    - `45` failed
    - all `45` failures were `missing_resolved_composer_id`
  - targeted composer seeding from `scripts/seed-imslp-work-composers.ts --offset 3200 --batch-size 100` then created `42` IMSLP-linked composers:
    - `failedWorkRows = 45`
    - `uniqueMissingComposers = 42`
    - `seedResults = { created: 42, updated: 0, flagged: 0, failed: 0 }`
  - direct candidate inspection from `scripts/inspect-imslp-work-slice.ts --offset 3200 --batch-size 100` then came back green:
    - `failureCount = 0`
  ...
  - canonical live row `f30d0039-c3c6-498c-816c-8ce0d51c0225` later settled green:
    ...
    - `90` quarantined
    - cursor advanced to `3300`
  - exact post-slice coverage audit from `scripts/audit-imslp-work-coverage.ts --offset-start 3200 --offset-end 3200 --step 100 --batch-size 100` passed after the live row settled:
    - `100` covered candidates
    - `0` uncovered candidates
    ...
    - duplicate-flag hygiene remained:
      ...
      - `0` duplicate-source collisions
  - operator note:
    - the first concurrent coverage audit run reported `100` uncovered candidates because it raced ahead of the live write
    - both initial dry-run rows also spent a misleading zero-counter period in `running` before backfilling their final results
    - the safer operating pattern remains “wait for the DB row to settle, then rerun exact coverage”
  - seeded audit from `scripts/sample-imslp-audit.ts --label offset-3200-postlive --sample-size 10 --seed 3200` sampled clean accepted orchestral works, IMSLP composers, and open quarantine rows
  - next step:
    - continue with the offset-`3300` recovery flow
...
- Strict IMSLP coverage and duplicate-review hygiene are now clean across offsets `1700` through `2900`:
  ...
  - next step:
    - keep using the strict coverage audit as the post-live gate while continuing beyond offset `3200`
