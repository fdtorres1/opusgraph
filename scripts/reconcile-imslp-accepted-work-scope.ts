...
function getPersistedInstrumentationText(row: WorkRow): string | null {
  ...
  if (!imslp || typeof imslp !== "object" || Array.isArray(imslp)) {
    return null;
  }

  const extractedFields = (imslp as Record<string, unknown>).extracted_fields;
...
function getImslpBucket(row: WorkRow): Record<string, unknown> | null {
...
async function main() {
  ...
  while (true) {
    ...
    for (const row of rows) {
      ...
      if (assessment.classification === "orchestral") {
        continue;
      }

      const imslp = getImslpBucket(row);
      const details = {
        source: "imslp",
        backfill: true,
        title: row.work_name,
        ...
        source_id:
          typeof imslp?.source_id === "string" ? imslp.source_id : null,
        source_url:
          typeof imslp?.source_url === "string" ? imslp.source_url : null,
      };
